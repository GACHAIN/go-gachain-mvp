...
func (c *Controller) Menu() (string, error) {
	...
	if len(utils.LangList) > 0 {
		for _, val := range utils.LangList {
			...
			if val == `zh` {
				langs = append(langs, LangInfo{Title: `Chinese (ZH)`, Code: `zh`})
			}
			if val == `nl` {
				langs = append(langs, LangInfo{Title: `Chinese (HK)`, Code: `hk`})
			}
		}
	} else {
		langs = []LangInfo{{Title: `English (UK)`, Code: `gb`},
			{Title: `Chinese (ZH)`, Code: `zh`},
			{Title: `Chinese (HK)`, Code: `hk`}}
	}
