...
import (
	...
	"github.com/GACHAIN/go-gachain-mvp/packages/daemons"
	"github.com/GACHAIN/go-gachain-mvp/packages/exchangeapi"
	"github.com/GACHAIN/go-gachain-mvp/packages/lib"
	"github.com/GACHAIN/go-gachain-mvp/packages/parser"
	"github.com/GACHAIN/go-gachain-mvp/packages/schema"
	"github.com/GACHAIN/go-gachain-mvp/packages/static"
	"github.com/GACHAIN/go-gachain-mvp/packages/stopdaemons"
	"github.com/GACHAIN/go-gachain-mvp/packages/system"
...
func Start(dir string, thrustWindowLoder *window.Window) {
	...
	go func() {
		...
		if len(configIni["db_type"]) > 0 {
			for {
				// wait while connection to a DB in other gourutina happens
				if utils.DB == nil || utils.DB.DB == nil {
					utils.Sleep(1)
					fmt.Println("wait DB")
				} else {
	...
	// waits for new records in chat, then waits for connects
	// (they are entered from the 'connections' daemon and from those who connected to the node by themselves)
